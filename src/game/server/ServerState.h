...
struct ObjectRef
{
	...
	sf_forceinline Object *operator->() const { return box.ptr; }
};
...
struct Handles
{
	sf::HashMap<ObjectId, sf::Box<Object>> objects;
};
...
struct State
{
	...
	void applyEvent(const Event &event, sf::Array<sf::Box<Event>> *undoList=nullptr);
};
