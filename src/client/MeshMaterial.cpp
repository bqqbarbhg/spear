...
bool MeshMaterial::useNormalMapRemap = false;
...
void MeshMaterial::globalInit()
{
	...
	sg_pixel_format normalFormat = MeshMaterial::materialFormats[(uint32_t)MaterialTexture::Normal];
	switch (normalFormat) {
	case SG_PIXELFORMAT_RG8:
	case SG_PIXELFORMAT_RGBA8:
	case SG_PIXELFORMAT_BC5_RG:
		MeshMaterial::useNormalMapRemap = false;
        break;
	case SG_PIXELFORMAT_BC3_RGBA:
	case SG_PIXELFORMAT_BQQ_ASTC_4X4_RGBA:
	case SG_PIXELFORMAT_BQQ_ASTC_8X8_RGBA:
		MeshMaterial::useNormalMapRemap = true;
		break;
	default:
		sf_failf("Unexpected format: %u", (uint32_t)normalFormat);
	}
}
